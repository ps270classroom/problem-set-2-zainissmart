...
party_incumbent_promote <- fb_ads |>
  ...
  head() |>
  knitr::kable(col.names = c("Party", "Incumbency", "Promotion"), digits = 3)
...
house_candidates <- fb_ads |>
  group_by(cand_name_last) |>
  filter(office == "US House")
house_candidates
  
plot_q3 <- ggplot(data = house_candidates, mapping = aes(x = cand_name_last,
                                               y = impressions_millions)) +
geom_col() 
...
```{r}
fb_top_plot <- ggplot(data = fb_ads, mapping = aes(x = impressions_millions,
                                                   y = ))
```
